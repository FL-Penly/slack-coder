...
class UpdateChecker:
    ...
    async def _check_loop(self) -> None:
        ...
        while self._running:
            ...
            self._reload_config()
            interval = max(
                self.config.check_interval_minutes, MIN_CHECK_INTERVAL_MINUTES
            )
    ...
    async def _do_check(self) -> None:
        ...
        try:
            ...
            self.state.last_check_at = time.strftime(
            ...
            if self.config.notify_slack and self.state.notified_version != latest:
                ...
                self.state.notified_at = time.strftime(
                    "%Y-%m-%dT%H:%M:%SZ", time.gmtime()
                )
                self.state.save()
    ...
    def _has_active_sessions(self) -> bool:
        ...
        try:
            ...
            if (
                hasattr(self.controller, "claude_sessions")
                and self.controller.claude_sessions
            ):
                return True
            ...
            if hasattr(self.controller, "agent_service"):
                ...
                if (
                    codex
                    and hasattr(codex, "active_processes")
                    and codex.active_processes
                ):
                    return True
        except Exception as e:
    ...
    async def _send_slack_notification(self, current: str, latest: str) -> None:
        ...
        if not dm_channel:
            logger.warning(
                f"Cannot send update notification: failed to open DM with {owner_id}"
            )
            return
        ...
        try:
            ...
            blocks = [
                {
                    ...
                    "text": {
                        "type": "mrkdwn",
                        "text": f":rocket: *Slack Coder Update Available*\n\n"
            ...
            await im_client.web_client.chat_postMessage(
                channel=dm_channel,
                text=f"Slack Coder update available: {current} → {latest}",
                blocks=blocks,
            )
        ...
        async with self._upgrade_lock:
            ...
            if result["ok"]:
                ...
                if result.get("restarting"):
                    ...
                    logger.warning(
                        "Upgrade completed without restart; manual restart required"
                    )
                return result
            else:
    ...
    async def check_and_send_post_update_notification(self) -> None:
        ...
        try:
            ...
            success_text = (
                f":white_check_mark: Slack Coder has been updated to `{target_version}`"
            )
            success_blocks = [
                {
                    "type": "section",
                    "text": {
                        "type": "mrkdwn",
                        "text": f":white_check_mark: *Slack Coder Updated Successfully*\n\n"
            ...
            if channel_id and message_ts:
                ...
                if owner_id:
                    ...
                    if dm_channel:
                        await im_client.web_client.chat_postMessage(
                            channel=dm_channel, text=success_text, blocks=success_blocks
                        )
...
async def handle_update_button_click(
    controller: "Controller", payload: Dict[str, Any]
) -> None:
    ...
    if (
        hasattr(controller, "update_checker")
        and controller.update_checker._upgrade_lock.locked()
    ):
        try:
            await im_client.web_client.chat_update(
                channel=channel_id,
    ...
    try:
        await im_client.web_client.chat_update(
            channel=channel_id,
            ts=message_ts,
            text="Updating Slack Coder...",
            blocks=[
                {
                    "type": "section",
                    "text": {
                        "type": "mrkdwn",
                        "text": ":hourglass_flowing_sand: *Updating Slack Coder...*\n\nPlease wait, the service will restart shortly.",
                    },
                }
            ],
        )
...
async def _do_update_from_button(
    controller: "Controller", channel_id: str, message_ts: str
) -> None:
                    ...
                    blocks=[
                        {
                            ...
                            "text": {
                                "type": "mrkdwn",
                                "text": ":x: *Slack Coder Upgrade Failed*\n\nPlease check the logs for details.",
                            },
                        }
                    ],
                )
                    ...
                    blocks=[
                        {
                            ...
                            "text": {
                                "type": "mrkdwn",
                                "text": ":white_check_mark: *Upgrade complete*\n\nPlease restart Slack Coder to apply the update.",
                            },
                        }
                    ],
                )
